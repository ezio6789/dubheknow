package com.insmess.knowledge.neo4j.wrapper;

import com.google.common.collect.Lists;
import com.google.common.collect.Maps;
import com.google.common.collect.Sets;
import lombok.Getter;
import com.insmess.knowledge.neo4j.utils.LambdaUtils;
...
public class Neo4jQueryWrapper<T> {
    ...
    /**
     * 分页，
     * @return 查询实体、关系、分页、跳阶、限制
     */
    public String getCypherPageQuery() {
        StringBuilder query = new StringBuilder("MATCH (n:" + getLabel() + ")").append("\n");
        if (!conditions.isEmpty()) {
            query.append(" WHERE ").append(String.join(" AND ", conditions)).append("\n");
        }
        query.append(" WITH n").append("\n");
        query.append(" ORDER BY id(n)").append("\n");
        query.append(" SKIP ").append(offset).append(" LIMIT ").append(limit).append("\n");
        //调用 APOC 查询多层子图
        query.append(" CALL apoc.path.subgraphAll(n, {").append("\n");
        query.append(" maxLevel: ").append(depthLevel).append(",").append("\n");
        query.append(" relationshipFilter: '>|<',").append("\n");
        query.append(" labelFilter: '',").append("\n");
        query.append(" bfs: true,").append("\n");
        query.append(" limit: ").append(relationLimit).append("\n");
        query.append(" })").append("\n");
        query.append(" YIELD nodes, relationships").append("\n");
        query.append(" RETURN nodes, relationships").append("\n");
     ...
     * @return 查询实体、关系、分页、跳阶、限制
     */
    public String getCypherByEntityIdQuery() {
        StringBuilder query = new StringBuilder();
        query.append(" MATCH (n)");
        query.append(" WHERE id(n) = $entityId");
        query.append(" CALL apoc.path.subgraphAll(n, {");
        query.append(" maxLevel: ").append(depthLevel).append(",");
        query.append(" relationshipFilter: '>|<',");
        query.append(" labelFilter: '',");
        query.append(" bfs: true,");
        query.append(" limit: ").append(relationLimit);
        query.append(" })");
        query.append(" YIELD nodes, relationships");
        query.append(" RETURN nodes, relationships;");
        return query.toString();
    }
     ...
     */
    public String getCypherStepQuery(Long id) {
        StringBuilder query = new StringBuilder("MATCH (n:" + getLabel() + ")");
        query.append(" WHERE id(n) = ").append(id);
        query.append(" CALL apoc.path.subgraphAll(n, {");
        query.append(" maxLevel: ").append(depthLevel).append(",");
        query.append(" relationshipFilter: '>|<',");
        query.append(" labelFilter: '" + getLabel() + "',");
        query.append(" bfs: true,");
        query.append(" limit: ").append(relationLimit);
        query.append(" })");
        query.append(" YIELD nodes, relationships");
