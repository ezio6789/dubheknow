package com.insmess.knowledge.database.query;

import com.insmess.knowledge.database.cache.DefaultSqlCache;
import com.insmess.knowledge.database.core.DbColumn;
import com.insmess.knowledge.database.core.DbTable;
import com.insmess.knowledge.database.core.PageResult;
...
public class CacheDbQueryFactoryBean extends AbstractDbQueryFactory {
    ...
    public List<Map<String, Object>> queryWithAliasPrefix(String tableA, String tableB, String joinCondition) {
        String cacheKey = sqlCache.buildSqlCacheKey(super.dataSource.toString() + ":" + tableA + ":" + tableB + ":" + joinCondition, null);
        return (List<Map<String, Object>>) Optional.ofNullable(sqlCache.get(cacheKey))
                .orElse(putCacheValue(cacheKey, super.queryWithAliasPrefix(tableA, tableB, joinCondition), DEFAULT_EXPIRE));
    }
