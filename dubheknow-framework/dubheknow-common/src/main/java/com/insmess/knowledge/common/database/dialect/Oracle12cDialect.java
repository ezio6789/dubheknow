package com.insmess.knowledge.common.database.dialect;
...
import com.insmess.knowledge.common.database.utils.MD5Util;
...
public class Oracle12cDialect extends OracleDialect {
    ...
    private List<String> generateOracleCreateSql(String tableName, String tableComment, List<DbColumn> columns) {
        ...
        for (DbColumn col : columns) {
            ...
            if (com.insmess.knowledge.common.utils.StringUtils.hasText(col.getDataDefault())) {
                String columnType = col.getDataType();
        ...
        // 表注释
        if (com.insmess.knowledge.common.utils.StringUtils.hasText(tableComment)) {
            String tableCmt = "COMMENT ON TABLE " + tableName + " IS '" + MD5Util.escapeSingleQuotes(tableComment) + "'";
            sqlList.add(tableCmt);
        }
        // 字段注释
        for (DbColumn col : columns) {
            if (com.insmess.knowledge.common.utils.StringUtils.hasText(col.getColComment())) {
