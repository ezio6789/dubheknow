<template>
  ...
  >
    ...
    >
      ...
      <el-table-column label="知识文档id" align="center" prop="documentId">
        <template #default="scope">
          {{ scope.row.documentId || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="知识内容" align="center" prop="content">
        <template #default="scope">
          {{ scope.row.content || '-' }}
        </template>
      </el-table-column>
...
<script setup name="KnowbaseDocumentContentSingle">
  import { listKnowbaseDocumentContent } from "@/api/knowbase/unstruct/knowbaseDocumentContent";
  import { ref } from "vue";
  ...
  function getList() {
    loading.value = true;
    listKnowbaseDocumentContent(proxy.addDateRange(queryParams.value, daterangeCreateTime.value)).then(
        async (response) => {
