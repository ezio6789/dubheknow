...
<script setup name="KnowbaseDocumentSingle">
  import { listKnowbaseDocument } from "@/api/knowbase/unstruct/knowbaseDocument";
  import { ref } from "vue";
  ...
  function getList() {
    loading.value = true;
    listKnowbaseDocument(proxy.addDateRange(queryParams.value, daterangeCreateTime.value)).then(
        async (response) => {
