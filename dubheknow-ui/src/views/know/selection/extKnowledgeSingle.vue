<template>
  ...
  >
    ...
    >
      ...
      <el-table-column label="vid类型" align="center" prop="vidType">
        <template #default="scope">
          {{ scope.row.vidType || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="知识图谱名称" align="center" prop="name">
        <template #default="scope">
          {{ scope.row.name || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="知识图谱描述" align="center" prop="description">
        <template #default="scope">
...
<script setup name="ExtKnowledgeSingle">
  import { listExtKnowledge } from "@/api/ext/extKnowledge/extKnowledge";
  import { ref } from "vue";
  ...
  function getList() {
    loading.value = true;
    listExtKnowledge(proxy.addDateRange(queryParams.value, daterangeCreateTime.value)).then(
        async (response) => {
