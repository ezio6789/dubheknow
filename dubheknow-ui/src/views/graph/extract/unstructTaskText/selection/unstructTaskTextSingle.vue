<template>
  ...
  >
    ...
    >
      ...
      <el-table-column label="段落标识" align="center" prop="paragraphIndex">
        <template #default="scope">
          {{ scope.row.paragraphIndex || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="文字内容" align="center" prop="text">
        <template #default="scope">
          {{ scope.row.text || '-' }}
        </template>
      </el-table-column>
...
<script setup name="UnstructTaskTextSingle">
  import { listUnstructTaskText } from "@/api/graph/extract/unstructTaskText";
  import { ref } from "vue";
  ...
  function getList() {
    loading.value = true;
    listUnstructTaskText(proxy.addDateRange(queryParams.value, daterangeCreateTime.value)).then(
        async (response) => {
