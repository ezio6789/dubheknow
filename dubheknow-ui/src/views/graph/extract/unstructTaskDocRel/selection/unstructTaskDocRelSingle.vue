<template>
  ...
  >
    ...
    >
      ...
      <el-table-column label="文件名" align="center" prop="docName">
        <template #default="scope">
          {{ scope.row.docName || '-' }}
        </template>
      </el-table-column>
...
<script setup name="UnstructTaskDocRelSingle">
  import { listUnstructTaskDocRel } from "@/api/graph/extract/unstructTaskDocRel";
  import { ref } from "vue";
  ...
  function getList() {
    loading.value = true;
    listUnstructTaskDocRel(proxy.addDateRange(queryParams.value, daterangeCreateTime.value)).then(
        async (response) => {
