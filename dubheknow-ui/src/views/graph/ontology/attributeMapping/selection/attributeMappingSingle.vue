...
<script setup name="AttributeMappingSingle">
  import { listAttributeMapping } from "@/api/graph/ontology/attributeMapping";
  import { ref } from "vue";
  ...
  function getList() {
    loading.value = true;
    listAttributeMapping(proxy.addDateRange(queryParams.value, daterangeCreateTime.value)).then(
        async (response) => {
