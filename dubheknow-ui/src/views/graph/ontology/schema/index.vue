    ...
    <div  class="pagecont-bottom">
     <div class="justify-between mb15">
       <el-row :gutter="15" class="btn-style">
         <el-col :span="1.5">
           <el-button type="primary" plain @click="handleAdd" v-hasPermi="['graph:ontology:schema:add']"
                      @mousedown="(e) => e.preventDefault()">
             <i class="iconfont-mini icon-xinzeng mr5"></i>新增
           </el-button>
         </el-col>
         <el-col :span="1.5">
           <el-button type="primary" plain :disabled="single" @click="handleUpdate" v-hasPermi="['graph:ontology:schema:edit']"
                      @mousedown="(e) => e.preventDefault()">
             <i class="iconfont-mini icon-xiugai--copy mr5"></i>修改
           </el-button>
         </el-col>
         <el-col :span="1.5">
           <el-button type="danger" plain :disabled="multiple" @click="handleDelete" v-hasPermi="['graph:ontology:schema:remove']"
                      @mousedown="(e) => e.preventDefault()">
             <i class="iconfont-mini icon-shanchu-huise mr5"></i>删除
           </el-button>
         </el-col>
         <el-col :span="1.5">
           <el-button type="info" plain  @click="handleImport" v-hasPermi="['graph:ontology:schema:export']"
                      @mousedown="(e) => e.preventDefault()">
             <i class="iconfont-mini icon-upload-cloud-line mr5"></i>导入
           </el-button>
         </el-col>
         <el-col :span="1.5">
           <el-button type="warning" plain @click="handleExport" v-hasPermi="['graph:ontology:schema:export']"
                      @mousedown="(e) => e.preventDefault()">
     ...
     <el-table stripe height="58vh" v-loading="loading" :data="schemaList" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
       <el-table-column type="selection" width="55" align="center" />
       <el-table-column v-if="getColumnVisibility(0)" label="ID" align="center" prop="id" />
       <el-table-column v-if="getColumnVisibility(1)" label="概念名称" align="center" prop="name">
         <template #default="scope">
           {{ scope.row.name || '-' }}
         </template>
       </el-table-column>
       <el-table-column v-if="getColumnVisibility(2)" label="概念描述" align="center" prop="description">
         <template #default="scope">
           {{ scope.row.description || '-' }}
         </template>
       </el-table-column>
       <el-table-column v-if="getColumnVisibility(3)" label="概念颜色" align="center" prop="color">
         <template #default="scope">
           {{ scope.row.color || '-' }}
         </template>
       </el-table-column>
       ...
       <el-table-column v-if="getColumnVisibility(13)" label="知识库id" align="center" prop="knowledgeId">
         <template #default="scope">
           {{ scope.row.knowledgeId || '-' }}
         </template>
       </el-table-column>
       <el-table-column v-if="getColumnVisibility(14)" label="父概念id" align="center" prop="pid">
         <template #default="scope">
           {{ scope.row.pid || '-' }}
         </template>
       </el-table-column>
       <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="240">
         <template #default="scope">
           <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                      v-hasPermi="['graph:ontology:schema:edit']">修改</el-button>
           <el-button link type="danger" icon="Delete" @click="handleDelete(scope.row)"
                      v-hasPermi="['graph:ontology:schema:remove']">删除</el-button>
           <el-button link type="primary" icon="view" @click="handleDetail(scope.row)"
                      v-hasPermi="['graph:ontology:schema:query']">详情</el-button>
           <el-button link type="primary" icon="view" @click="routeTo('/graph/ontology/schemaDetail',scope.row)"
                      v-hasPermi="['graph:ontology:schema:query']">复杂详情</el-button>
         </template>
       </el-table-column>
...
<script setup name="Schema">
  ...
  const upload = reactive({
    ...
    // 上传的地址
    url: import.meta.env.VITE_APP_BASE_API + "/graph/schema/importData"
  });
  ...
  function getList() {
    ...
    listSchema(queryParams.value).then(response => {
            schemaList.value = response.data.records;
