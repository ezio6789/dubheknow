<template>
  ...
  >
    ...
    >
      ...
      <el-table-column label="概念名称" align="center" prop="name">
      ...
      <el-table-column label="概念颜色" align="center" prop="color">
        <template #default="scope">
          {{ scope.row.color || '-' }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="父概念id" align="center" prop="pid">
        <template #default="scope">
          {{ scope.row.pid || '-' }}
        </template>
      </el-table-column>
...
<script setup name="SchemaSingle">
  ...
  function getList() {
    loading.value = true;
    listSchema(proxy.addDateRange(queryParams.value, daterangeCreateTime.value)).then(
        async (response) => {
