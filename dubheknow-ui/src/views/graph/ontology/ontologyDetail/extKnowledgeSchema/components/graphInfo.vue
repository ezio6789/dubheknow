  ...
  <div class="app-container" ref="app-container">
    ...
    >
      ...
      <el-form
          ...
          @submit.prevent
      >
        <el-row :gutter="20" v-show="radioShow">
          <el-col :span="12">
            <el-form-item label="节点类型">
              <el-radio-group v-model="radio1" @change="handleChange">
                <el-radio value="1" size="large">起点</el-radio>
                <el-radio value="2" size="large">终点</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
        </el-row>
...
const data = reactive({
  ...
  rules: {
    ...
    inverseFlag: [
      { required: true, message: "是否可逆不能为空", trigger: "blur" },
    ],
    validFlag: [
...
const handleChange = (e) => {
  switch (e) {
    case "1":
      changeStartEnd(props.nodeData.id);
      break;
    case "2":
      changeStartEnd(null, props.nodeData.id);
      break;
  }
};
...
watch(
    () => props,
    (newVal) => {
      if (Object.keys(newVal.nodeData).length > 0) {
        getList(newVal.nodeData.id);
...
function changeStartEnd(start = null, end = null) {
  form.value.startSchemaId = start;
  form.value.endSchemaId = end;
  disable1.value = start ? true : false;
  disable2.value = end ? true : false;
}
...
/** 提交按钮 */
function submitForm() {
  form.value.knowledgeId = knowledgeId.value;
